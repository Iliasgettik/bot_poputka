...
async def process_and_publish_ad(text_to_analyze: str, message: types.Message, msgs_to_delete: list = None):
    ...
    КАК ОТЛИЧИТЬ ВОДИТЕЛЯ ОТ ПАССАЖИРА (ОЧЕНЬ ВАЖНО):
    ...
    - ПРАВИЛО: Если он пишет марку авто (Kia K5) и пишет "чыгам" — это "айдоочу".

    ▶ Маркеры "жүк ташуу" (Грузоперевозки):
    - Слова: "грузовой спринтер", "уй коч", "мебель", "жук ташуу".

    3. ОСОБЕННОСТИ КЫРГЫЗСКОГО:
    Слова "чыгам", "чыгабыз", "барам", "кетем" используют ВСЕ. Не определяй роль только по ним! Смотри на контекст (ищет машину или предлагает пустой салон). Если просто написано "Бишкектен Таласка чыгабыз 2 киши" и непонятно, машина это или пассажиры — ставь role: null.
